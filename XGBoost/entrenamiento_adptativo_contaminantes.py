...
class EntrenadorAdaptativoContaminantes:
    ...
    def _validacion_leave_one_out(self, X, y, param_dist, features, contaminante):
        """Validación Leave-One-Out para datasets muy pequeños (≤ 6 muestras)"""
        
        print(f"   🔍 Leave-One-Out para dataset extremo ({len(X)} muestras)")
        
        # Para datasets de 2 muestras, usar validación simple
        if len(X) <= 2:
            return self._validacion_minimal(X, y, param_dist, features, contaminante)
        ...
        # Probar solo algunas combinaciones de parámetros
        for i in range(5):  # Muy pocas iteraciones para evitar overfitting
            # Generar parámetros conservadores
            params = {}
            ...
            try:
                ...
                scores = cross_val_score(
                    modelo, X_scaled, y, 
                    cv=loo, 
                    scoring=make_scorer(f1_score, average='binary', zero_division=0),
                    n_jobs=1
                )
        ...
        # Si no se encontraron parámetros válidos, usar parámetros por defecto
        ...
        print(f"   ✅ Mejor LOO score: {mejor_score:.4f}")
        
        # Entrenar modelo final con todos los datos
        modelo_final = xgb.XGBClassifier(**mejores_params, random_state=42, verbosity=0)
        modelo_final.fit(X_scaled, y)
        
        # Para LOO, train=test (no hay división real)
        return self._evaluar_modelo_final_loo(modelo_final, X_scaled, y, features, contaminante, mejores_params)
    
    def _validacion_stratified_cv(self, X, y, param_dist, features, contaminante):
        """Validación cruzada estratificada para datasets pequeños-medianos"""
        
        print(f"   🔍 Stratified CV para dataset ({len(X)} muestras)")
        
        # Para datasets muy pequeños, usar holdout
        if len(X) <= 4:
            return self._validacion_holdout_robusto(X, y, param_dist, features, contaminante)
        ...
        n_folds = min(3, len(X_train) // 2)
        n_folds = max(2, n_folds)
        
        mejor_score = -np.inf
        mejores_params = None
        
        # Búsqueda de parámetros con CV
        for i in range(10):  # Más iteraciones para datasets un poco más grandes
            params = {}
            for key, dist in param_dist.items():
                if hasattr(dist, 'rvs'):
                    params[key] = dist.rvs()
        ...
        # Parámetros por defecto si falla
        if mejores_params is None:
            mejores_params = {
                'n_estimators': 8,
                'max_depth': 3,
        ...
        print(f"   ✅ Mejor CV score: {mejor_score:.4f}")
    ...
    def _validacion_minimal(self, X, y, param_dist, features, contaminante):
        """Validación mínima para datasets de 2 muestras"""
        
        print(f"   🚨 Dataset MINIMAL: Solo {len(X)} muestras - Validación simplificada")
        
        # Usar parámetros ultra conservadores
        params_minimal = {
            'n_estimators': 2,      # Solo 2 árboles
            'max_depth': 2,         # Profundidad mínima
            'learning_rate': 0.3,   # Learning rate estándar
            'reg_alpha': 10.0,      # Regularización máxima
            'reg_lambda': 15.0,     # Regularización máxima
            'min_child_weight': 1   # Peso mínimo para permitir splits
        }
        
        # Entrenar modelo simple
        modelo_minimal = xgb.XGBClassifier(**params_minimal, random_state=42, verbosity=0)
        
        try:
            modelo_minimal.fit(X, y)
            y_pred = modelo_minimal.predict(X)
            
            # Para datasets de 2 muestras, train=test
            accuracy = accuracy_score(y, y_pred)
            f1 = f1_score(y, y_pred, zero_division=0)
            
            # Feature importance si está disponible
            importance = {}
            if hasattr(modelo_minimal, 'feature_importances_') and len(features) > 0:
                for i, imp in enumerate(modelo_minimal.feature_importances_):
                    if i < len(features):
            ...
            return {
                ...
                'metodo': 'validacion_minimal',
                'tipo_quimico': perfil.get('tipo', 'desconocido'),
                'categoria_quimica': perfil.get('categoria_quimica', 'desconocido'),
                'complejidad_modelo': perfil.get('model_complexity', 'minimo'),
                
                # Métricas (train=test en este caso)
                'test_accuracy': float(accuracy),
                'test_f1': float(f1),
                'train_accuracy': float(accuracy),  # Mismos valores
                'train_f1': float(f1),              # Mismos valores
                'gap_accuracy': 0.0,                # Sin gap real
                'gap_f1': 0.0,                      # Sin gap real
                'auc': 0.5,                         # AUC no calculable
                'diagnostico_overfitting': 'NO_EVALUABLE',
                
                # Configuración
                'parametros_especificos': params_minimal,
                'features_utilizados': features,
                'n_features': len(features),
                'feature_importance': importance,
                'estrategia_validacion': 'minimal',
                
                # Metadatos
                'n_muestras_train': len(X),
                'n_muestras_test': len(X),
                'notas': 'Dataset extremadamente pequeño - Resultados limitados'
            }
            
        except Exception as e:
            print(f"   ❌ Error en validación minimal: {e}")
            return None
    
    def _evaluar_modelo_final_loo(self, modelo, X_scaled, y, features, contaminante, params):
        """Evaluación especial para Leave-One-Out (sin división train/test real)"""
        
        # En LOO no hay división real train/test
        ...
        return {
            'contaminante': contaminante,
            'metodo': 'leave_one_out',
            'tipo_quimico': perfil.get('tipo', 'desconocido'),
            'categoria_quimica': perfil.get('categoria_quimica', 'desconocido'),
            'complejidad_modelo': perfil.get('model_complexity', 'minimo'),
            
            # En LOO, train=test efectivamente
            'test_accuracy': float(accuracy),
            'test_f1': float(f1),
            'train_accuracy': float(accuracy),
            'train_f1': float(f1),
            'gap_accuracy': 0.0,  # No hay gap real en LOO
            'gap_f1': 0.0,        # No hay gap real en LOO
            'auc': float(auc),
            'diagnostico_overfitting': 'LOO_NO_EVALUABLE',
            ...
            'n_features': len(features),
            'feature_importance': importance,
            'estrategia_validacion': 'leave_one_out',
            
            # Metadatos
            'n_muestras_train': len(X_scaled),
            'n_muestras_test': len(X_scaled),
            'notas': 'Leave-One-Out: train=test, gap no evaluable'
        }
